...
const moveFns = async (
  ...
  const imports = [];
  imports.push('const Player = common.Player;');
  imports.push('const Result = common.Result;');
  imports.push('const Effectiveness = data.Effectiveness;');
  imports.push('const Move = data.Move;');
  imports.push('const Effects = mechanics.Effects;');
  imports.push('const State = mechanics.State;');
  ...
  for (const command of IMPORTS) {
    imports.push(`const ${command} = mechanics.${command};`);
  }

  for (const line of imports.sort((a, b) =>
...
const GEN: {[gen in GenerationNum]?: GenerateFn} = {
  1: async (gen, dirs, update, tests) => {
    ...
    template('moves', dirs.out, {
      gen: gen.num,
      roman: 'I',
      import: 'const rng = @import("../../common/rng.zig");',
