...
test "turn order (complex speed tie)" {
    ...
    // beforeTurnMove
    {
        ...
        try expectEqual(Result.Default, try t.update(move(1), move(1)));
        // (1/2) * (255/256) * (255/256)
        try if (showdown) t.expectProbability(65025, 131072) else t.expectProbability(1, 2);
...
test "choices" {
    var random = rng.PSRNG.init(0x2718281);
...
test "FreezeChance effect" {
    ...
    try if (showdown) t.expectProbability(255, 256) else t.expectProbability(1, 1);
    ...
    try t.expectFinalProbability(3, 8);
...
test "Paralyze effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    // (255/256) * (3/4) * (191/256) vs. (3/4) * (191/256)
    try if (showdown) t.expectProbability(146115, 262144) else t.expectProbability(573, 1024);
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    // (216/256) * (3/4) * (191/256) vs. (3/4) * (191/256)
    try if (showdown) t.expectProbability(15471, 32768) else t.expectProbability(573, 1024);
...
test "Sleep effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    try t.expectFinalProbability(255, 1792); // (255/256) * (1/7)
    try expectEqual(0, t.actual.p2.get(1).status);
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    try t.expectFinalProbability(765, 896); // (255/256) * (6/7)
    try expectEqual(t.expected.p2.get(1).status, t.actual.p2.get(1).status);
    ...
    try t.expectFinalProbability(29161, 7667712); // (241/256) * (1/39) * (242/256) * (1/6)
    try expectEqual(0, t.actual.p2.get(1).status);
...
test "Confusion effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(2), move(2)));
    try t.expectFinalProbability(765, 2048); // (3/4) * (1/2) * (255/256)
    ...
    try expectEqual(Result.Default, try t.update(move(2), move(2)));
    try t.expectFinalProbability(85, 256); // (1/3) * (255/256)
    try expect(!t.actual.p2.active.volatiles.Confusion);
...
test "ConfusionChance effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    try t.expectFinalProbability(17935, 3407872); // (1/4) * (255/256) * (211/256) * (1/39)
    try expect(!t.actual.p2.active.volatiles.Confusion);
...
test "Charge effect" {
    ...
    try t.expectFinalProbability(245, 512); // (7/8) * (140/256)
    try expectEqual(pp - 1, t.actual.p1.active.move(2).pp);
...
test "Binding effect" {
    ...
    try expectEqual(Result.Default, try t.update(forced, forced));
    try t.expectFinalProbability(15, 32); // (3/4) * (5/8)
...
test "Thrashing effect" {
    ...
    // normal
    {
        ...
        try t.expectFinalProbability(535740975, 5806795784192);
        try expectEqual(pp - 1, t.actual.p1.active.move(1).pp);
        try expect(t.actual.p1.active.volatiles.Confusion);
        try expectEqual(5, t.actual.p1.active.volatiles.confusion);
        ...
        if (showdown) {
            // (3/4) ** 2 * (1/2) * (255/256) ** 2 * (231/256) * (1/39)
            try t.expectFinalProbability(45062325, 6979321856);
        } else {
            // (3/4) ** 2 * (1/2) * (255/256) * (231/256) * (1/39)
            try t.expectFinalProbability(176715, 27262976);
        }
    ...
    // immune
    {
        ...
        try expectEqual(Result.Default, try t.update(forced, move(1)));
        try t.expectFinalProbability(1, 2);
        try expect(t.actual.p1.active.volatiles.Confusion);
        try expectEqual(5, t.actual.p1.active.volatiles.confusion);
...
test "Disable effect" {
    ...
    try t.expectFinalProbability(20825, 54525952);
    ...
    try t.expectFinalProbability(145775, 40894464);
    ...
    try t.expectFinalProbability(245, 2048); // (140/256) * (1/4) * (7/8)
    ...
    try t.expectFinalProbability(8925, 851968);
    ...
    // Haze clears disable
    try expectEqual(Result.Default, try t.update(move(2), move(2)));
    try t.expectFinalProbability(45475, 2555904); //  (255/256) * (214/256) * (1/39) * (5/6)
    try expectEqual(0, t.actual.p2.active.volatiles.disable_move);
...
test "Counter effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    if (showdown) {
        // (255/256) ** 2 * (206/256) * (1/39) * (230/256)
        try t.expectProbability(256740375, 13958643712);
    } else {
        // (255/256) * (206/256) * (1/39) * (230/256)
    ...
    try expectEqual(Result.Default, try t.update(move(3), move(1)));
    // (255/256) ** 2
    try if (showdown) t.expectProbability(65025, 65536) else t.expectProbability(1, 1);
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    try t.expectFinalProbability(573, 896); // (191/256) * (6/7)
    ...
    try expectEqual(Result.Default, try t.update(move(2), forced));
    try t.expectFinalProbability(5, 6);
...
test "DreamEater effect" {
    ...
    // substitute, the user does not recover any HP.
    {
        ...
        try expectEqual(Result.Default, try t.update(move(2), move(1)));
        try t.expectFinalProbability(459, 896); // (153/256) * (6/7)
        ...
        try expectEqual(Result.Default, try t.update(move(1), forced));
        try t.expectFinalProbability(94775, 5111808); // (255/256) * (223/256) * (1/39) * (5/6)
...
test "LeechSeed effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(1), move(1)));
    // (229/256) * (27/256)
    try if (showdown) t.expectProbability(27, 256) else t.expectProbability(6183, 65536);
...
test "Rage effect" {
    ...
    try t.expectFinalProbability(7, 2048); // (7/8) * (1/256)
...
test "Bide effect" {
    ...
    // Wave, Toxic, or Transform, the previous damage dealt to the user will be added to the total.
    {
        ...
        try expectEqual(Result.Default, try t.update(forced, move(1)));
        try t.expectFinalProbability(255, 512); // (255/256) * (1/2)
...
test "Metronome effect" {
    ...
    try expectEqual(Result.Default, try t.update(move(2), forced));
    try t.expectFinalProbability(1, 512); // (1/2) * (1/256)
    ...
    if (showdown) {
        ...
        try t.expectFinalProbability(1194429775, 308561514380394496);
    } else {
        // (7/8) * (1/4) * (1/163) * (255/256) * (226/256) * (1/39)
        try t.expectFinalProbability(67235, 2221932544);
    }
...
test "Confusion self-hit bug" {
    ...
    try expectEqual(Result.Default, try t.update(move(2), move(1)));
    try t.expectFinalProbability(3, 8); // (3/4) * (1/2)
...
test "Disable + Bide bug" {
    ...
    try expectEqual(Result.Default, try t.update(move(2), forced));
    try t.expectFinalProbability(245, 1024); // (140/256) * (1/2) * (7/8)
    try expectEqual(4, t.actual.p2.active.volatiles.disable_duration);
    try expect(t.actual.p2.active.volatiles.Bide);
    try expectEqual(3, t.actual.p2.active.volatiles.attacks);
    ...
    try expectEqual(Result.Default, try t.update(move(3), forced));
    try t.expectFinalProbability(6, 7);
    try expectEqual(3, t.actual.p2.active.volatiles.disable_duration);
    try expect(t.actual.p2.active.volatiles.Bide);
    try expectEqual(3, t.actual.p2.active.volatiles.attacks);
...
test "Charge + Sleep bug" {
    ...
    try expectEqual(Result.Default, try t.update(forced, move(2)));
    try t.expectFinalProbability(1, 7);
...
test "Bide + Substitute bug" {
    ...
    try expectEqual(Result.Default, try t.update(move(1), forced));
    try t.expectFinalProbability(229, 512); // (229/256) * (1/2)
...
test "Counter + sleep = Desync Clause Mod bug" {
    ...
    try t.expectFinalProbability(765, 896); // (6/7) * (255/256)
...
test "Counter via Metronome bug" {
    ...
    // Counter first
    {
        ...
        try expectEqual(Result.Default, try t.update(move(1), move(2)));
        if (showdown) {
            try t.expectProbability(65025, 10682368); // (1/163) * (255/256) ** 2
        } else {
    ...
    // Counter second
    {
        ...
        try expectEqual(Result.Default, try t.update(move(1), move(1)));
        if (showdown) {
            try t.expectProbability(65025, 10682368); // (255/256) ** 2 * (1/163)
        } else {
...
test "Counter glitches" {
    ...
    // Desync
    {
        ...
        // (1/4) * (255/256) vs. (1/4)
        try if (showdown) t.expectProbability(255, 1024) else t.expectProbability(1, 4);
        try t.verify();
    }
}
...
test "Poison/Burn animation with 0 HP" {
    ...
    // Faint from confusion self-hit (heals on Pokémon Showdown)
    {
        ...
        try expectEqual(Result{ .p1 = .Pass, .p2 = .Switch }, try t.update(move(2), move(1)));
        try t.expectFinalProbability(687, 2048); // (229/256) * (3/4) * (1/2)
        try t.verify();
    }
}
...
test "Hyper Beam + Sleep move glitch" {
    ...
    try expectEqual(Result.Default, try t.update(move(3), forced));
    try t.expectFinalProbability(1, 6);
    try expectEqual(t.expected.p2.get(1).status, t.actual.p2.get(1).status);
...
test "Trapping sleep glitch" {
    ...
    try expectEqual(Result.Default, try t.update(move(2), forced));
    // (191/256) * (6/7)
    try if (showdown) t.expectFinalProbability(573, 896) else t.expectProbability(1, 1);
    try expectEqual(t.expected.p2.get(2).status, t.actual.p2.get(1).status);
...
test "Rage and Thrash / Petal Dance accuracy bug" {
    ...
    // should miss!
    try expectEqual(Result.Default, try t.update(forced, move(1)));
    try t.expectFinalProbability(43, 128); // (1/2) * (172/256)
...
test "Substitute + Confusion glitch" {
    ...
    // Confused Pokémon has Substitute
    {
        ...
        try if (showdown) t.expectFinalProbability(105, 512) else t.expectFinalProbability(3, 8);
        try expectEqual(7, t.actual.p1.active.volatiles.substitute);
        ...
        try if (showdown) t.expectFinalProbability(35, 192) else t.expectFinalProbability(1, 3);
    ...
    // Both Pokémon have Substitutes
    {
        ...
        try expectEqual(Result.Default, try t.update(move(2), move(2)));
        try t.expectFinalProbability(1, 3); // (2/3) * (1/2)
        try expectEqual(7, t.actual.p1.active.volatiles.substitute);
...
test "MIN_CHANCE" {
    ...
    try t.expectFinalProbability(3179172198123, 10110943703216766844928);
...
test "RNG overrides" {
    ...
    // Full
    {
        const Random = extern struct {
            ...
            pub fn confusionDuration(_: @This(), _: Player) u3 {
                return 2;
            }

            pub fn attackingDuration(_: @This(), _: Player) u3 {
                return 3;
            }
            ...
            pub fn advance(_: @This(), _: usize) void {}
        };
        ...
        var battle: data.Battle(Random) = .{
            ...
            .sides = .{
                ...
                helpers.Side.init(&.{.{
                    .species = .Chansey,
                    .moves = &.{ .Metronome, .Psywave, .Barrage },
                }}),
            },
        };
        ...
        p1.get(1).hp -= 52;
        try expected.damage(.{ P1.ident(1), p1.get(1), .None });
        p1.get(1).status = Status.init(.PSN);
        ...
        try expected.move(.{ P1.ident(1), Move.ThunderShock, P2.ident(1) });
        p2.get(1).hp -= 32;
        try expected.damage(.{ P2.ident(1), p2.get(1), .None });
        p2.get(1).status = Status.init(.PAR);
        try expected.status(.{ P2.ident(1), p2.get(1).status, .None });
        p1.get(1).hp -= 43;
        try expected.damage(.{ P1.ident(1), p1.get(1), .Poison });
        try expected.end(.{ P2.ident(1), .Confusion });
        try expected.move(.{ P2.ident(1), Move.Psywave, P1.ident(1) });
        p1.get(1).hp -= 1;
        try expected.damage(.{ P1.ident(1), p1.get(1), .None });
        try expected.turn(.{4});
        ...
        try expected.move(.{ P1.ident(1), Move.Disable, P2.ident(1) });
        try expected.start(.{ P2.ident(1), .Disable, Move.Psywave });
        p1.get(1).hp -= 43;
        try expected.damage(.{ P1.ident(1), p1.get(1), .Poison });
        try expected.end(.{ P2.ident(1), .Disable });
        try expected.move(.{ P2.ident(1), Move.Barrage, P1.ident(1) });
        p1.get(1).hp -= 19;
        try expected.damage(.{ P1.ident(1), p1.get(1), .None });
        p1.get(1).hp -= 19;
        try expected.damage(.{ P1.ident(1), p1.get(1), .None });
        ...
        try expected.move(.{ P1.ident(1), Move.Metronome, P1.ident(1) });
        try expected.move(.{ P1.ident(1), Move.Psychic, P2.ident(1), Move.Metronome });
        p2.get(1).hp -= 72;
        try expected.damage(.{ P2.ident(1), p2.get(1), .None });
        try expected.boost(.{ P2.ident(1), .SpecialAttack, -1 });
        try expected.boost(.{ P2.ident(1), .SpecialDefense, -1 });
        p1.get(1).hp -= 43;
        try expected.damage(.{ P1.ident(1), p1.get(1), .Poison });
        try expected.move(.{ P2.ident(1), Move.Metronome, P2.ident(1) });
        try expected.move(.{ P2.ident(1), Move.Thrash, P1.ident(1), Move.Metronome });
        p1.get(1).hp -= 107;
        try expected.damage(.{ P1.ident(1), p1.get(1), .None });
        try expected.turn(.{6});
    ...
    if (showdown) {
        const Random = extern struct {
            ...
            pub fn range(self: *@This(), comptime T: type, from: T, to: rng.PSRNG.Bound(T)) T {
                return self.rng.range(T, from, to);
            }
            ...
            pub fn chance(
                ...
                comptime T: type,
                numerator: T,
                denominator: rng.PSRNG.Bound(T),
            ) bool {
                return self.rng.chance(T, numerator, denominator);
            }

            pub fn next(_: @This()) u8 {
                // TODO: ziglang/zig analysis issue...
                unreachable;
            }
        };

        var battle: data.Battle(Random) = .{
            .rng = .{ .rng = rng.PSRNG.init(0x12345678) },
            .sides = .{
                helpers.Side.init(&.{.{
                    .species = .Zapdos,
                    .moves = &.{.Thunder},
        ...
        try expected.move(.{ P2.ident(1), Move.BodySlam, P1.ident(1) });
        p1.get(1).hp -= 102;
        try expected.damage(.{ P1.ident(1), p1.get(1), .None });
        p1.get(1).status = Status.init(.PAR);
        try expected.status(.{ P1.ident(1), p1.get(1).status, .None });
        try expected.move(.{ P1.ident(1), Move.Thunder, P2.ident(1) });
        p2.get(1).hp -= 191;
        try expected.damage(.{ P2.ident(1), p2.get(1), .None });
        try expected.turn(.{2});
